...
function Connect-GPDBrowser {
    ...
    if (-not $ChromePath) {
        $ChromePath = Get-GPDChromePath
    }

    if (-not $ChromePath -and -not $AttachOnly) {
        throw 'Could not find chrome.exe. Supply -ChromePath.'
    }
    ...
    try {
        ...
        if ($AttachOnly) {
            throw "Chrome is not listening on 127.0.0.1:$RemoteDebuggingPort. Start Chrome with --remote-debugging-port=$RemoteDebuggingPort or omit -AttachOnly."
        }
        ...
        if ($NewWindow) { $args = @('--new-window') + $args }

        Write-Verbose "Starting Chrome: $ChromePath $($args -join ' ')"
        $script:GPDState.ChromeProcess = Start-Process -FilePath $ChromePath -ArgumentList $args -PassThru
    ...
    if (-not $target) {
        throw 'Could not open or locate a Google Photos Chrome target.'
    }
...
function Start-GPDLibraryScan {
    ...
    for ($scroll = 0; $scroll -lt $MaxScrolls; $scroll++) {
        ...
        foreach ($item in $normalised) {
            if (-not $item.LocalId) { continue }
            if ($existingIds.Add([string]$item.LocalId)) {
                $newRecords.Add($item)
            }
        }
        ...
        Write-Progress -Activity 'Scanning Google Photos timeline' -Status "Scroll $($scroll + 1)/$MaxScrolls - new this round: $($newRecords.Count), total new: $totalNew" -PercentComplete ((($scroll + 1) / [Math]::Max(1, $MaxScrolls)) * 100)
        Write-Verbose "Scroll $($scroll + 1): visible=$($rawItems.Count), new=$($newRecords.Count), no-new-rounds=$roundsWithoutNew"

        if ($StopWhenNoNewItems -and $roundsWithoutNew -ge $NoNewItemRounds) {
            Write-Verbose "Stopping after $roundsWithoutNew rounds without new items."
            break
        }
...
function Resolve-GPDMediaItem {
    ...
    if ($LocalId) {
        $set = [System.Collections.Generic.HashSet[string]]::new([string[]]$LocalId, [System.StringComparer]::OrdinalIgnoreCase)
        $items = @($items | Where-Object { $_.LocalId -and $set.Contains([string]$_.LocalId) })
    }
    if ($ItemUrl) {
        $set = [System.Collections.Generic.HashSet[string]]::new([string[]]$ItemUrl, [System.StringComparer]::OrdinalIgnoreCase)
        $items = @($items | Where-Object { $_.ItemUrl -and $set.Contains([string]$_.ItemUrl) })
    }
    if ($MediaKey) {
        $set = [System.Collections.Generic.HashSet[string]]::new([string[]]$MediaKey, [System.StringComparer]::OrdinalIgnoreCase)
        $items = @($items | Where-Object { $_.MediaKey -and $set.Contains([string]$_.MediaKey) })
    }
    if ($VisualHash64) {
        $items = @($items | Where-Object { $_.VisualHash64 -eq $VisualHash64 })
    }
    if ($ThumbnailUrl) {
        $items = @($items | Where-Object { $_.ThumbnailUrl -eq $ThumbnailUrl })
    }
    if ($PSBoundParameters.ContainsKey('ApproxDate')) {
        $dateText = $ApproxDate.ToString('yyyy-MM-dd')
        $items = @($items | Where-Object { $_.ApproxDate -eq $dateText })
    }
    if ($Kind) {
        $items = @($items | Where-Object { $_.Kind -eq $Kind })
    }
...
function Get-GPDDuplicateGroup {
    ...
    param(
        [Parameter(Mandatory)]
        [string]$DatabasePath,
        [ValidateSet('ExactVisualHash','Conservative','Relaxed')]
        [string]$Similarity = 'Conservative',
    ...
    $exactThumbGroups = $items |
        Where-Object { $_.ThumbnailUrl } |
        Group-Object ThumbnailUrl |
        Where-Object { $_.Count -gt 1 }

    foreach ($group in $exactThumbGroups) {
        $groupNumber++
        $members = @($group.Group | Sort-Object ApproxDate, LocalId)
        $groups.Add((New-GPDDuplicateGroupObject -GroupNumber $groupNumber -Reason 'SameThumbnailUrl' -Members $members))
    }

    if ($Similarity -eq 'ExactVisualHash') {
        $hashGroups = $items |
            Where-Object { $_.VisualHash64 } |
            Group-Object Kind, VisualHash64 |
            Where-Object { $_.Count -gt 1 }

        foreach ($group in $hashGroups) {
            $groupNumber++
            $members = @($group.Group | Sort-Object ApproxDate, LocalId)
            $groups.Add((New-GPDDuplicateGroupObject -GroupNumber $groupNumber -Reason 'SameVisualHash64' -Members $members))
        }
    ...
    $buckets = $items |
        Where-Object { $_.VisualHash64 -and $_.ApproxDate -and $_.AspectBucket } |
        Group-Object Kind, ApproxDate, AspectBucket

    foreach ($bucket in $buckets) {
        $bucketItems = @($bucket.Group)
        if ($bucketItems.Count -lt 2) { continue }
        if ($bucketItems.Count -gt $MaxBucketSize) {
            Write-Warning "Skipping oversized comparison bucket '$($bucket.Name)' with $($bucketItems.Count) items. Increase -MaxBucketSize if needed."
            continue
        }
...
function Export-GPDReport {
    ...
    $cards = foreach ($group in $groups) {
        $members = foreach ($item in $group.Members) {
            ...
            $keeper = if ($item.LocalId -eq $group.ProposedKeeperLocalId) { '<span class="keeper">KEEPER</span>' } else { '<span class="deleteCandidate">candidate</span>' }
            $img = if ($safeThumb) { "<img src='$safeThumb' loading='lazy' alt='thumbnail' />" } else { '<div class="noThumb">No thumbnail</div>' }
            $link = if ($safeUrl) { "<a href='$safeUrl' target='_blank' rel='noreferrer'>Open</a>" } else { '<span>No link</span>' }
            "<article class='item'>$img<div class='meta'>$keeper<div><b>$safeKind</b> $safeDate</div><code>$safeId</code><code>$safeHash</code><div>$link</div></div></article>"
        }
...
<style>
:root { color-scheme: dark; --bg:#101018; --panel:#181827; --hot:#ff4fd8; --cyan:#25d9ff; --text:#f8f8ff; --muted:#b8b8cc; --good:#7CFF6B; --warn:#ffcc66; }
body { margin:0; font-family: Segoe UI, Roboto, Arial, sans-serif; background: radial-gradient(circle at top left, #32104b, var(--bg) 40%), var(--bg); color:var(--text); }
header { padding:32px; background: linear-gradient(120deg, rgba(255,79,216,.25), rgba(37,217,255,.18)); border-bottom:1px solid rgba(255,255,255,.12); }
h1 { margin:0; font-size: clamp(2rem, 4vw, 4rem); }
.summary { color:var(--muted); font-size:1.1rem; }
.group { margin:24px; padding:18px; border:1px solid rgba(255,255,255,.12); background:rgba(24,24,39,.9); border-radius:18px; box-shadow:0 12px 30px rgba(0,0,0,.25); }
h2 { margin:0 0 8px; } h2 span { color:var(--cyan); font-size:.9rem; }
.items { display:grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap:14px; }
.item { border:1px solid rgba(255,255,255,.10); border-radius:14px; overflow:hidden; background:#0d0d15; }
.item img, .noThumb { width:100%; height:210px; object-fit:cover; background:#05050a; display:flex; align-items:center; justify-content:center; color:var(--muted); }
.meta { padding:12px; display:grid; gap:6px; }
code { display:block; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; color:#d7d7ff; background:#07070b; padding:4px; border-radius:6px; }
a { color:var(--cyan); }
.keeper { color:var(--good); font-weight:800; } .deleteCandidate { color:var(--warn); font-weight:700; }
</style>
</head>
<body>
<header><h1>Google Photos Duplicate Review</h1><p class="summary">Generated $generated · Groups: $($groups.Count) · Similarity: $Similarity</p></header>
<main>
$($cards -join "`n")
</main>
</body>
...
function Remove-GPDMediaItem {
    ...
    process {
        ...
        if ($matches.Count -eq 0) {
            throw 'No Google Photos media item matched the supplied identifier properties.'
        }
        ...
        foreach ($item in $matches) {
            if (-not $item.ItemUrl) {
                throw "Item $($item.LocalId) has no ItemUrl. Cannot safely delete through the browser UI. Re-scan with item opening enabled when that feature is added."
            }
...
function Invoke-GPDDeduplication {
    ...
    if ($Scan) {
        $result.Scan = Start-GPDLibraryScan -DatabasePath $DatabasePath -MaxScrolls $MaxScrolls -ThrottleMs $ThrottleMs -HashConcurrency $HashConcurrency -StopWhenNoNewItems
    }
    if ($FindDuplicates) {
        $result.DuplicateGroups = @(Get-GPDDuplicateGroup -DatabasePath $DatabasePath -Similarity Conservative)
    }
    if ($ExportReportPath) {
        $result.Report = Export-GPDReport -DatabasePath $DatabasePath -Path $ExportReportPath -Open:$OpenReport
    }
...
function Invoke-GPDDetractorReview {
    ...
    $moduleFile = Join-Path $PSScriptRoot 'Google-PhotosDeDuplicate.psm1'
    $manifestFile = Join-Path $PSScriptRoot 'Google-PhotosDeDuplicate.psd1'

    foreach ($path in @($moduleFile, $manifestFile)) {
        if (-not (Test-Path -LiteralPath $path)) {
            $findings.Add([pscustomobject]@{ Severity = 'Error'; Rule = 'RequiredFile'; Message = "Missing file: $path" })
        }
    }
    ...
    if (Test-Path -LiteralPath $moduleFile) {
        ...
        if ($content -notmatch 'SupportsShouldProcess') {
            $findings.Add([pscustomobject]@{ Severity = 'Error'; Rule = 'DeletionSafety'; Message = 'Remove-GPDMediaItem should support ShouldProcess.' })
        }
        if ($content -notmatch 'Identifier properties matched') {
            $findings.Add([pscustomobject]@{ Severity = 'Error'; Rule = 'AmbiguitySafety'; Message = 'Deletion should refuse ambiguous identifier matches.' })
        }
        if ($content -match 'password\s*=|client_secret|refresh_token') {
            $findings.Add([pscustomobject]@{ Severity = 'Error'; Rule = 'SecretPattern'; Message = 'Potential secret-like string found in module.' })
        }
        if ($content -notmatch 'remote-debugging-port') {
            $findings.Add([pscustomobject]@{ Severity = 'Warning'; Rule = 'ChromeCDP'; Message = 'Could not find Chrome remote debugging configuration.' })
        }
    }

    if ($findings.Count -eq 0) {
        [pscustomobject]@{ Passed = $true; FindingCount = 0; Findings = @() }
    }
    else {
...
function Get-GPDChromePath {
    ...
    $cmd = Get-Command chrome, google-chrome, google-chrome-stable, chromium -ErrorAction SilentlyContinue | Select-Object -First 1
...
function Invoke-GPDRest {
    ...
    Invoke-RestMethod -Uri $uri -Method $Method -TimeoutSec 10
}
...
function New-GPDChromeTarget {
    ...
    Invoke-GPDRest -Path "/json/new?$encoded" -Port $Port -Method Put
}
...
function Assert-GPDBrowserConnected {
    ...
    if (-not $script:GPDState.ActiveWebSocketUrl) {
        throw 'Not connected to Chrome. Run Connect-GPDBrowser first.'
    }
}
...
function Invoke-GPDChromeEvaluate {
    ...
    Invoke-GPDWebSocketMessage -WebSocketUrl $script:GPDState.ActiveWebSocketUrl -JsonMessage $message -TimeoutSeconds $TimeoutSeconds
}
...
function Invoke-GPDWebSocketMessage {
    ...
    try {
        ...
        do {
            ...
            $receive = $socket.ReceiveAsync($segment, $cts.Token).GetAwaiter().GetResult()
        ...
        if ($response.result.result.subtype -eq 'error') { throw $response.result.result.description }
...
(async () => {
  ...
  const mediaKeyFromUrl = (url) => {
    if (!url) { return null; }
    const m = url.match(/\/photo\/([^/?#]+)/) || url.match(/\/video\/([^/?#]+)/);
  ...
  for (const img of imgs) {
    ...
    if (!src || !/googleusercontent|photos/i.test(src)) { continue; }
    ...
    const kind = /video|movie|play/i.test(aria) ? 'Video' : 'Photo';
    records.push({
      Kind: kind,
      ItemUrl: itemUrl,
      MediaKey: mediaKeyFromUrl(itemUrl),
      ThumbnailUrl: src,
      VisualHash64: await hashImage(img),
      ApproxDate: normaliseDate(aria),
      AriaLabel: aria,
...
'@
    ...
    $result = Invoke-GPDChromeEvaluate -Expression $script -TimeoutSeconds 90
    @($result)
}
...
function Read-GPDMediaStore {
    ...
    Get-Content -LiteralPath $DatabasePath -ReadCount 1000 | ForEach-Object {
        foreach ($line in $_) {
            ...
            if ($record.RecordType -eq 'MediaItem' -and $record.LocalId) {
                $latest[[string]$record.LocalId] = $record
            }
...
function Get-GPDHammingDistanceHex {
    ...
    for ($i = 0; $i -lt $Left.Length; $i++) {
        $a = [Convert]::ToInt32($Left[$i].ToString(), 16)
        $b = [Convert]::ToInt32($Right[$i].ToString(), 16)
        $x = $a -bxor $b
        while ($x -gt 0) {
            $distance += ($x -band 1)
            $x = $x -shr 1
...
function New-GPDDuplicateGroupObject {
    ...
    [pscustomobject]@{
        GroupId               = ('G{0:000000}' -f $GroupNumber)
        Reason                = $Reason
        Confidence            = if ($Reason -eq 'SameThumbnailUrl') { 100 } elseif ($Reason -eq 'SameVisualHash64') { 95 } else { 80 }
        Count                 = $orderedMembers.Count
        ProposedKeeperLocalId = $orderedMembers[0].LocalId
...
(async () => {
  ...
  const clickMatching = (patterns) => {
    const candidates = Array.from(document.querySelectorAll('button, [role="button"], div[aria-label], span[aria-label]')).filter(isVisible);
...
'@
    ...
    Add-GPDJsonLine -DatabasePath $DatabasePath -InputObject @($audit)

    if (-not $result.ok) {
        throw "Browser deletion failed for $($Item.LocalId): $($result.stage)"
    }
